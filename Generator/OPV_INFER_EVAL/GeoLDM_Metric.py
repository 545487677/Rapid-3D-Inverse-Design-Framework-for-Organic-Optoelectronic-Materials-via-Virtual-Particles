...
def calculate_metrics(path, sample_length, train_data_path, seed):
    ...
    sdf_files = glob.glob('{}/**/*.txt'.format(path), recursive=True)
    # for folder in subdir:
    #     search_pattern = os.path.join(folder, '/**/*.xyz')
    #     sdf_files.extend(glob.glob(search_pattern))
    print(len(sdf_files))
    sdf_files = sdf_files[:sample_length]
    # for file in sdf_files:
    #     smi = get_valid_smiles_from_xyz(file) 
    #     smi_list.append(smi) 
    for file in sdf_files:
...
# Example usage 
path = '../Baseline_Generated/generated_mols/OPV/GeoLDM' 
sample_length = 10000 
