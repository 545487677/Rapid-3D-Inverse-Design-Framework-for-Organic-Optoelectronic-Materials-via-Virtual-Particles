...
from .chunk_layer import _chunk_slice
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.norm = 1 / math.sqrt(self.c_hidden)  # [1]
    ...
    def forward(
        ...
        if biases is not None:
            for b in biases:
                attn = attn + b
        a = softmax_dropout(attn, 0, self.training)
...
class GlobalAttention(nn.Module):
    ...
    def forward(self, m: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        bias = gen_attn_mask(mask, -self.inf)[..., :, None, :]
        a = a + bias
        a = softmax_dropout(a, 0, self.training)
...
def inplace_add(x, y, inplace):
    if torch.is_grad_enabled() or not inplace:
        x = x + y
    else:
        x += y
    return x