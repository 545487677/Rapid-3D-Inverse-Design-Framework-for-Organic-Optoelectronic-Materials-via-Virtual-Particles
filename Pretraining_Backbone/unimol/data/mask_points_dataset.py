...
class MaskPointsDataset(BaseWrapperDataset):
    ...
    def __getitem_cached__(self, epoch: int, index: int):
        ...
        with data_utils.numpy_seed(self.seed, epoch, index):
            ...
            num_mask = max(num_mask, min(self.min_num_mask, sz))
            ...
            if unmask is not None:
                ...
                if new_mask.sum() >= self.min_num_mask:
                    mask = new_mask
...
class MaskPointsWithFeatDataset(BaseWrapperDataset):
    def __init__(
        self,
        dataset: torch.utils.data.Dataset,
        atom_feat_dataset: torch.utils.data.Dataset,
        coord_dataset: torch.utils.data.Dataset,
        vocab: Dictionary,
        pad_idx: int,
        mask_idx: int,
        ...
        self.dataset = dataset
        self.atom_feat_dataset = atom_feat_dataset
        self.coord_dataset = coord_dataset
        self.vocab = vocab
        self.pad_idx = pad_idx
        self.mask_idx = mask_idx
    ...
    def set_epoch(self, epoch, **unused):
        super().set_epoch(epoch)
        self.coord_dataset.set_epoch(epoch)
        self.atom_feat_dataset.set_epoch(epoch)
        self.dataset.set_epoch(epoch)
        self.epoch = epoch
    ...
    def __getitem_cached__(self, epoch: int, index: int):
        ...
        with data_utils.numpy_seed(self.seed, epoch, index):
            ...
            new_atom_feat_num = np.copy(self.atom_feat_dataset[index]['atom_feat_num'])
            new_atom_feat_num[mask] = 0.0

            new_atom_feat_cat = np.copy(self.atom_feat_dataset[index]['atom_feat_cat'])
            new_atom_feat_cat[mask] = 0
            ...
            ret["atoms"] = torch.from_numpy(new_item).long()
            ret["atom_feat_num"] = torch.from_numpy(new_atom_feat_num).float()
            ret["atom_feat_cat"] = torch.from_numpy(new_atom_feat_cat).long()
            ret["coordinates"] = torch.from_numpy(new_coord).float()
            return ret
        
class MaskPointsWithChargeDataset(BaseWrapperDataset):
    def __init__(
        self,
        dataset: torch.utils.data.Dataset,
        ...
        self.charge_noise_f = lambda num_mask: np.random.uniform(
                low=-self.charge_noise, high=self.charge_noise, size=num_mask,
            )
        self.min_num_mask = min_num_mask
    ...
    def __getitem_cached__(self, epoch: int, index: int):
        ...
        with data_utils.numpy_seed(self.seed, epoch, index):
            ...
            new_atoms_charge = np.copy(self.charge_dataset[index])
            new_atoms_charge[mask] += self.charge_noise_f(num_mask)
            ...
            ret["atoms"] = torch.from_numpy(new_item).long()
            ret["atoms_charge"] = torch.from_numpy(new_atoms_charge).float()
            ret["coordinates"] = torch.from_numpy(new_coord).float()
            return ret