...
class PairChargeDataset(BaseWrapperDataset):
    ...
    @lru_cache(maxsize=16)
    def __getitem__(self, index: int):
        atoms_charge = self.dataset[index]
        sz = atoms_charge.size(0)

        pair_charge = torch.stack([atoms_charge.view(sz, 1).expand(sz, sz), 
                                    atoms_charge.view(1, sz).expand(sz, sz)], dim=2)
        return pair_charge