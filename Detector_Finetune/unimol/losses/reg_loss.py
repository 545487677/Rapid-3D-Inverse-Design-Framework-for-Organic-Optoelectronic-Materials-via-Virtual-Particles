...
class FinetuneSmoothMAELoss(FinetuneMSELoss):
    ...
    #     weights = torch.tensor([self.args.weight_constant if i == 0 else 1.0 for i in range(predicts.size(1))], device=predicts.device)

    #     losses = []
    #     for i in range(predicts.size(1)):
    #         loss = F.smooth_l1_loss(predicts[:, i], targets[:, i])
    #         weighted_loss = weights[i] * loss
    #         losses.append(weighted_loss)

    #     loss = sum(losses)

    #     return loss if reduce else (loss, *losses)
