...
@register_model("unimol_debug")
class UniMolDebugModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, dictionary):
        ...
        # for i in range(1):
        #     head_name = f'class_{i}' 
        #     self.classification_heads[head_name] = ClassificationHead(
        ...
        #         pooler_dropout=self.args.pooler_dropout,
        #     )
        self.predictor1 = ClassificationHead(
                input_dim=self.args.encoder_embed_dim,
                inner_dim=self.args.encoder_embed_dim, 
                num_classes=1, 
                activation_fn=self.args.pooler_activation_fn,
        ...
        self.predictor2 = ClassificationHead(
                input_dim=self.args.encoder_embed_dim,
                inner_dim=self.args.encoder_embed_dim, 
                num_classes=1, 
                activation_fn=self.args.pooler_activation_fn,
    ...
    def forward(
        ...
        **kwargs
        ...
        (
            encoder_rep1,
            encoder_pair_rep1,
            delta_encoder_pair_rep1,
            x_norm1,
            delta_encoder_pair_rep_norm1,
        ) = self.encoder1(x, padding_mask=padding_mask, attn_mask=graph_attn_bias)
        (
            encoder_rep2,
            encoder_pair_rep2,
            delta_encoder_pair_rep2,
            x_norm2,
            delta_encoder_pair_rep_norm2,
        ) = self.encoder2(x, padding_mask=padding_mask, attn_mask=graph_attn_bias)



        encoder_pair_rep1[encoder_pair_rep1 == float("-inf")] = 0
        encoder_pair_rep2[encoder_pair_rep2 == float("-inf")] = 0        
        ...
        homo_pred = self.predictor1(encoder_rep1)
        lumo_pred = self.predictor2(encoder_rep2)

        logits = torch.cat((homo_pred, lumo_pred), dim=1)




        if self.args.mode == 'infer':
            return encoder_rep1, encoder_pair_rep1
        else:
            return (
                logits,
                encoder_distance,
                encoder_coord,
                None, #x_norm,
                None # delta_encoder_pair_rep_norm,
            )         
...
@register_model_architecture("unimol_debug", "unimol_debug")
...
@register_model_architecture("unimol_debug", "unimol_debug_base")
